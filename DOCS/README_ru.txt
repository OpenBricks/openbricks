...
Вы можете найти эти кодеки в пакете на http://www.mplayerhq.hu/MPlayer/releases/codecs/.

Полезные файлы в этом пакете:
   - Real кодеки (используются для проигрывания rv9, например): atrc.so.6.0, cook.so.6.0, sipr.so.6.0, drv3.so.6.0, drv4.so.6.0.
   - Windows Media кодеки (wmv9/wma9): wma9dmod.dll и wmv9dmod.dll.
...
Язык - можно легко выбрать предпочитаемый Вами язык меню, следуя приведенной ниже процедуре:
   ...
   Отредактируйте скрипт generator.sh перед его выполнением и измените строку в начале:    "LANG=en" на предпочитаемый Вами язык. Например, если Вы француз, измените это так "LANG=fr".
...
Здесь есть также улучшенные команды, если Вы хотите подробнее разобраться с GeeXboX:
  ...
  make exec                    # запустить непосредственно GeeXboX в ограничении
                               # ПРЕДУПРЕЖДЕНИЕ: это особо экспериментальная характеристика
                               # используйте это на Ваш собственный риск
