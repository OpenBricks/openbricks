...
cgi_cookie_type * cgi_cookies = NULL;
int cgi_num_entries = 0;
int cgi_num_cookies = 0;
	  ...
	  if (getenv("CONTENT_TYPE") == NULL)
			 ...
			 "boundary=") == NULL)
		{
		  ...
		  return(cgi_errno);
              ...
              strcpy(boundary,
                     strstr(getenv("CONTENT_TYPE"), "boundary=") +
                     strlen("boundary="));
				  ...
				  if (strstr(cgi_query,
				      ...
				      if (strstr(cgi_query, "name=\"") != NULL)
					{
					  ...
					  cgi_entries[which_entry].name =
					    malloc(strlen(strstr(cgi_query,
						   "name=")) -
						   strlen("name=") + 1);
	  ...
	  if (buf_tmp == NULL)
	  ...
	  else
	    {
		 ...
		 string is: */
	      ...
	      cgi_query = malloc(sizeof(char) * (cl + 1));
	      if (cgi_query == NULL)
	      {
		cgi_errno = CGIERR_OUT_OF_MEMORY;
		return(cgi_errno);
  ...
  do
    {
	 ...
	 leave us with a ';' at the start of cookie */
      ...
      num_cookies++;
      name = malloc((size_t)MAX_COOKIE_STUFF);
      ...
      if(sscanf(cookie,sscanf_string,name,value) != 2)
	{
      ...
      tmp_cookie = realloc((void *)cgi_cookies,
